...
function greedyMakeChange(target, coins = [25, 10, 5, 1]) {
  ...
  const largestCoin = coins.find(coin => coin <= target);
  const remainder = target - largestCoin;
  const result = greedyMakeChange(remainder, coins);

  return [largestCoin, ...result];
}
...
function makeBetterChange(target, coins = [25, 10, 5, 1]) {
  ...
  coins.forEach(coin => {
    if (coin <= target) {
      ...
      const currentChange = [coin, ...makeBetterChange(remainder, coins)];
